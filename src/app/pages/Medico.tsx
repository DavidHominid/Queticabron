...
export function Medico() {
  ...
  const handleSubmitConsulta = async (e: React.FormEvent) => {
    ...
    if (
      ...
      try {
        const dia = String(seguimientoHorario.dia || '').trim();
        const hora = String(seguimientoHorario.horaInicio || '').trim();
        if (!dia || !hora) throw new Error('Selecciona un bloque para agendar el seguimiento.');
        if (dia <= hoy) throw new Error('El seguimiento debe agendarse en un día futuro.');

        const espEvento = eventoSeguimiento.especialidades.find((e) => e.especialidad === especialidadSeguimiento) || null;
        if (!espEvento) throw new Error('No se encontró la especialidad en el evento seleccionado.');

        const cupoTotal = Number.isFinite(Number(seguimientoHorario.cupoTotal)) ? Math.max(0, Math.floor(Number(seguimientoHorario.cupoTotal))) : 0;
        const ocupadas = (citas || []).filter((c) => {
          if (c.estado === 'cancelada') return false;
          if (String(c.eventoId) !== String(eventoSeguimiento.id)) return false;
          if (String(c.especialidad) !== String(especialidadSeguimiento)) return false;
          if (String(c.fecha) !== dia) return false;
          const tipoId = String(seguimientoHorario.tipoCitaId || '').trim();
          if (tipoId && String(c.tipoCitaId || '') !== tipoId) return false;
          return c.hora === hora;
        }).length;
        if (cupoTotal > 0 && ocupadas >= cupoTotal) throw new Error('Ese bloque ya no tiene cupos disponibles.');

        const tipos = Array.isArray((espEvento as any).tiposCita) ? ((espEvento as any).tiposCita as any[]) : [];
        const tipoPreferido = String(seguimientoHorario.tipoCitaId || selectedCita.tipoCitaId || '').trim();
        const tipo = tipoPreferido ? tipos.find((t) => String(t?.id || '') === tipoPreferido) || null : null;
        const tipoFinal = tipo || (tipos[0] || null);

        const conflictoPaciente = (citas || []).some((c) => {
          if (c.estado === 'cancelada') return false;
          if (String(c.pacienteId) !== String(selectedCita.pacienteId)) return false;
          if (String(c.eventoId) !== String(eventoSeguimiento.id)) return false;
          if (String(c.fecha) !== dia) return false;
          if (String(c.hora) !== hora) return false;
          return true;
        });
        if (conflictoPaciente) throw new Error('Ese horario ya está ocupado para este paciente.');
        ...
        const nuevaCita = {
          ...
          tipoCitaId: tipoFinal?.id ? String(tipoFinal.id) : undefined,
          tipoCitaNombre: tipoFinal?.nombre ? String(tipoFinal.nombre) : undefined,
          duracionMinutos: Number.isFinite(Number(tipoFinal?.duracionMinutos)) ? Number(tipoFinal.duracionMinutos) : undefined,
          medicoEncargado: String(tipoFinal?.medicoEncargado || '').trim() || String((espEvento as any).medicoEncargado || '').trim() || '',
          estado: 'programada',
          costoPagado: Number.isFinite(Number(tipoFinal?.precio))
            ? Number(tipoFinal.precio)
        ...
        await addCita(nuevaCita as any);
        ...
        if (updateSeguimiento) {
          await updateSeguimiento(String(saved.id), {
            ...
            estado: 'agendada',
            diagnostico: consultaForm.diagnostico,
            observaciones: consultaForm.recomendaciones || '',
            fechaCita: dia,
            horaCita: hora,
        ...
        addRegistroAuditoria({
          ...
          detalles: `Agendó seguimiento para ${selectedCita.paciente?.nombre || 'paciente'} · ${dia} ${hora} · Evento: ${eventoSeguimiento.nombre} (cita ID: ${nuevaCita.id})`,
          fechaHora: nowIso(),
          ciudad: user?.ciudad || eventoSeguimiento.ciudad || 'sonoyta',
        });
      } catch (err: any) {
        const msg = err?.message || 'No se pudo agendar el seguimiento. Quedará como pendiente.';
        setSeguimientoMensaje(msg);
        alert(msg);
      }
    }
